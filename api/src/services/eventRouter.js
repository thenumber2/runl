...
class EventRouter {
  ...
  async _sendToDestination(event, destination, webhookForwarder) {
    try {
      ...
      if (!isRegistered) {
        webhookForwarder.registerDestination(destination.name, {
          url: destination.url,
      ...
      webhookForwarder.registerDestination(identityDestination.name, identityDestination);
      const result = await webhookForwarder.processEvent(tempEvent);

      // Clean up temporary destination
      webhookForwarder.removeDestination(identityDestination.name);
